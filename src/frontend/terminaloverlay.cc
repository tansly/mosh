...
void NotificationEngine::apply( Framebuffer &fb ) const
{
  ...
  if ( message.empty() && time_expired ) {
    swprintf( tmp, 128, L"mosh: Last %s %s:%s %s ago.%s", explanation,
    ...
    swprintf( tmp, 128, L"mosh: %ls (%s without %s %s:%s.)%s", message.c_str(),
	      human_readable_duration( static_cast<int>( time_elapsed ),
				       "s" ).c_str(),
	      explanation, server_ip.c_str(), server_port.c_str(), keystroke_str );
  }
